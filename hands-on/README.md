...
## 🤖 4. Antigravity CLI (`agy`)를 통한 자동화 및 검증 사용법

본 실습 저장소는 Google의 에이전트 개발 도구인 **Antigravity CLI (명령어: `agy`)** 환경에 최적화된 **에이전트 하네스(Agent Harness)** 설정이 내장되어 있습니다. 이를 통해 코드를 복사-붙여넣기 하지 않고 슬래시 명령어 하나만으로 코드를 자동 생성하고 로컬 서버 구동 여부를 진단 및 검증할 수 있습니다.

### 1) Antigravity CLI 기동
터미널에서 프로젝트 루트 디렉토리로 이동한 뒤 `agy` 명령어를 입력하여 인터랙티브 세션을 시작합니다:
```bash
agy
```

### 2) 슬래시 명령어(워크플로우)를 통한 코드 자동 생성
세션이 시작되면 아래 슬래시 명령어를 입력하여 제약조건 및 규칙에 부합하는 코드를 즉시 자동 생성할 수 있습니다:
*   `/gemma-coder`: 기본 대화 스트리밍 및 생각 과정(Thinking) 파싱 코드 자동 생성 (`hands-on/main.py`)
*   `/gemma-fc`: 실시간 무료 날씨 API(`wttr.in`)와 연동되는 Function Calling 코드 자동 생성 (`hands-on/function_calling.py`)
*   `/gemma-so`: JSON 스키마를 따르는 Structured Output 코드 자동 생성 (`hands-on/structured_output.py`)
*   `/gemma-tt`: `--no-think` CLI 인자에 따라 동작하는 생각 과정 필터링 코드 자동 생성 (`hands-on/thinking_toggle.py`)
*   `/flutter-coder`: DartPad용 단일 파일 Flutter 앱 코드 자동 생성 (`hands-on/flutter_app.dart`)

### 3) 로컬 환경 및 LLM 서버 상태 진단
로컬 서버(Ollama, LM Studio, llama.cpp)가 잘 구동 중인지 또는 파이썬 가상환경 의존성이 갖춰져 있는지 에이전트에게 진단을 명령할 수 있습니다:
*   **입력 예시:** `"로컬 개발 환경 및 LLM 포트 상태 진단해줘"` 또는 `"스킬을 사용해서 개발 환경 체크해줘"`
*   **동작 스킬:** `gemma-local-diagnostic` 스킬이 자동으로 실행되어 포트 상태, 로드된 모델 목록, 파이썬 라이브러리 정상 설치 여부를 리포트합니다.

### 4) 생성된 소스코드 무결성 및 연동 검증
생성한 코드에 에러가 없으며 로컬 서버와 정상 스트리밍 연동이 되는지 검증을 명령할 수 있습니다:
*   **입력 예시:** `"방금 만든 function_calling.py가 잘 돌아가는지 검증해줘"`
*   **동작 스킬:** `hands-on-verifier` 스킬이 작동하여 구문 오류(Syntax Check)를 분석하고, 로컬 LLM 서버에 연결하여 테스트 응답 스트리밍을 수행해 통과 여부를 검증합니다.
