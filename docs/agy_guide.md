...
내가 작성한 작업 공간 규칙과 커스텀 에이전트가 `agy` CLI에 정상적으로 로드되었는지 확인하려면 `agy` CLI를 실행한 후 세션 내에서 다음 슬래시 명령어를 입력하여 확인합니다:

*   **등록된 에이전트 확인:** `/agents` 명령어를 통해 `gemma-coder`, `gemma-fc` 등의 에이전트가 정상적으로 등록되었는지 확인합니다.
*   **슬래시 명령어(워크플로우) 확인:** CLI 창에 슬래시(`/`) 또는 `/help`를 입력하여 우리가 추가한 `/gemma-coder`, `/gemma-fc` 등의 워크플로우 명령어가 목록에 정상 노출되는지 확인합니다.
