...
## 도구별 사전 준비 가이드
사용 환경이나 선호하는 도구에 맞춰 아래 세 가지 설치 가이드 중 하나를 선택해 진행해 주세요. (가장 대중적이고 쉬운 방법은 **Ollama** 또는 **LM Studio입니다.**)
...
### 4. [uv 설치 및 개발 환경 가이드](./uv_guide.md)
*   **추천 대상:** 실습용 파이썬 환경을 초고속으로 구축하고 패키지 의존성을 관리하고 싶으신 분.
*   **특징:** Rust 기반 패키지 매니저로, 가상환경 구성 및 `httpx` 등의 라이브러리 초고속 설치 가능.

### 5. [DartPad 사용 및 설정 가이드](./dartpad_guide.md)
*   **추천 대상:** 로컬 환경에 Flutter SDK 등을 설치하지 않고, 웹 브라우저에서 바로 플러터 앱을 실행하고 실습하고 싶으신 분.
*   **특징:** 별도 다운로드나 컴파일 없이 즉시 웹 브라우저 환경에서 단일 파일 플러터 코드를 기동하고 결과를 확인할 수 있음.

---

## 추가 참고 자료
*   [Gemma 4 모델 상세 사양 안내](./gemma4_guide.md): Gemma 4의 특징(텍스트/이미지/오디오 멀티모달, 함수 호출 등)과 아키텍처에 대한 상세 정보입니다.
*   [핸즈온 실습 코드 및 시스템 프롬프트](../hands-on/README.md): Flutter/DartPad 개발 및 로컬 Gemma 4 API(Ollama, LM Studio, llama.cpp) 호출을 위한 환경변수 기반 파이썬 비동기 실습(httpx) 코드와 AI 질의용 시스템 프롬프트(Flutter 웜업, 기본 스트리밍, Function Calling, Structured Output, 생각 과정 토글)입니다.
*   [AGY SDK를 활용한 핸즈온 코드 생성 가이드](./agy_guide.md): Google Antigravity (AGY) SDK 에이전트를 구성하고 핸즈온 시스템 프롬프트를 연동하여 코드를 자동 생성하는 방법 안내입니다.
